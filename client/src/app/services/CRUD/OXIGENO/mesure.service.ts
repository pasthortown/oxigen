...
export class MesureService {
   ...
   post(mesure: Mesure): Promise<any> {
      return this.http.post(this.url, JSON.stringify(mesure), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }

   put(mesure: Mesure): Promise<any> {
      return this.http.put(this.url, JSON.stringify(mesure), this.options).toPromise()
      .then( r => {
         return r.json();
      }).catch( error => { this.handledError(error.json()); });
   }
